...
export const mcp_list_collections = async () => {
    try {
...
export const mcp_analyze_collection_schema = async (params) => {
    try {
        ...
        const dataTypes = {};
        const nestedStructures = new Map();
...
function analyzeFields(obj, prefix, fieldStats) {
...
function analyzeDocumentSchema(obj, prefix, fieldAnalysis, dataTypes, nestedStructures, totalDocs) {
    for (const [key, value] of Object.entries(obj)) {
        const fieldPath = prefix ? `${prefix}.${key}` : key;
...
function generateIndexSuggestions(fields) {
    const suggestions = [];
